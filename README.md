...
**痛点一：用英文和 Claude 交流，效果更好，但口语跟不上。**

Claude 的训练数据以英文为主，这直接影响了它在不同语言下的输出质量。同样一个问题，英文版本得到的回答在推理深度、细节完整度、表达精准度上，通常明显优于中文版本。这不是主观感受，是大量用户长期使用后的一致反馈。于是很多人开始刻意用英文和 Claude 交互，但新问题出现了：口语表达跟不上，想说的说不清楚，反而拖慢了节奏。

**痛点二：等待时间没有好的利用方式。**

用 Claude 做事的过程中，等待是常态。发出一个 prompt，然后等——10 秒、30 秒、有时候更长。这段时间太短，不值得打开另一件事；太长，又只是发呆。
...
语音转写使用本地 Whisper 模型，Ghost Coach 分析使用 Kimi 账号——两者均完全免费。
