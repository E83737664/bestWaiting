...
你每天花多少时间在等 Claude 回复？

10 秒、30 秒、有时候更长。这段时间大多数人只是盯着进度条。**Best Waiting** 把这段时间变成练英文的机会。

## 问题

用 Claude 做事的过程中，等待是常态。你发出一个 prompt，然后等。等的时候没有一个自然的事情可以做——太短，不够打开另一件事；太长，又只是发呆。
